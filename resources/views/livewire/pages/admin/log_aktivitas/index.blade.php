            ...
            <div class="bg-surface-dark border border-[#3E4C59] rounded-xl shadow-2xl min-h-[300px]">
                <table class="w-full">
                    <thead class="bg-gray-900 sticky top-0 z-10">
                        <tr>
                            <th class="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-[#3E4C59]">Waktu</th>
                            <th class="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-[#3E4C59]">User & Akun</th>
                            <th class="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-[#3E4C59]">Action</th>
                            <th class="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-[#3E4C59]">Target</th>
                            <th class="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-[#3E4C59]">Deskripsi</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($this->logs as $log)
                            ...
                            <tr class="hover:bg-surface-hover transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <p class="text-slate-500 text-[11px] font-mono tracking-tighter">{{ $log->created_at->format('H:i:s') }}</p>
                                </td>

                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="relative">
                                            <div class="w-10 h-10 rounded-xl flex items-center justify-center border {{ $role['class'] }} shadow-lg">
                                                <span class="font-black text-sm uppercase">{{ substr($log->user?->name ?? 'S', 0, 1) }}</span>
                                            </div>
                                            <div class="absolute -bottom-1 -right-1 w-3.5 h-3.5 border-2 border-surface-dark rounded-full {{ $isActive ? 'bg-emerald-500' : 'bg-red-500' }}"></div>
                                        </div>

                                        <div>
                                            <div class="flex items-center gap-2">
                                                <p class="text-white text-sm font-bold italic leading-none">{{ $log->user?->name ?? 'SYSTEM' }}</p>
                                                <span class="text-slate-500 text-[11px] font-mono lowercase tracking-tighter">@ {{ $log->user?->username ?? 'system' }}</span>
                                            </div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-[11px] text-slate-300 font-mono bg-gray-900/50 px-2 py-1 rounded border border-[#3E4C59]">
                                        {{ $log->target ?? '-' }}
                                    </span>
                                </td>

                                <td class="px-6 py-4 max-w-xs">
                                    <p class="text-xs text-slate-400 leading-relaxed group-hover:text-slate-200 transition-colors">
                                        {{ $log->description }}
                                    </p>
                                </td>
                            </tr>
